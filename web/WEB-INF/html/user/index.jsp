    ...
    <div class="fly-panel fly-panel-user" pad20>
        ...
        <div class="layui-tab layui-tab-brief" lay-filter="user">
            <ul class="layui-tab-title" id="LAY_mine">
                <li data-type="mine-jie" lay-id="index" class="layui-this">我发的帖（<span>${loader.publishedList.size()}</span>）</li>
                <li data-type="collection" data-url="/collection/find/" lay-id="collection">我收藏的帖（<span>${loader.collectionList.size()}</span>）</li>
            </ul>
            <div class="layui-tab-content" style="padding: 20px 0;">
                <div class="layui-tab-item layui-show">
                    <ul class="mine-view jie-row">
                        <c:if test="${loader!=null}">
                            <c:forEach items="${loader.publishedList}" var="published">
                        <li>
                            <a class="jie-title" href="${pageContext.servletContext.contextPath}/detail?id=${published.id}" target="_blank">${published.title}</a>
                            <i>${published.publishTime}</i>
                            <a class="mine-edit" href="/jie/edit/8116">编辑</a>
                            <em>${published.views}阅/${published.replyNum}答</em>
                        </li>
                            </c:forEach>
                        </c:if>
<%--                        <li>--%>
...
<%--                        </li>--%>
                ...
                <div class="layui-tab-item">
                    <ul class="mine-view jie-row">
                        <c:if test="${loader!=null}">
                        <c:forEach items="${loader.collectionList}" var="collection">
                        <li>
                            <a class="jie-title" href="${pageContext.servletContext.contextPath}/detail?id=${collection.aid}" target="_blank">${collection.title}</a>
                            <i>${collection.collectedTime}</i>  </li>
                        </c:forEach>
                        </c:if>
                    </ul>
